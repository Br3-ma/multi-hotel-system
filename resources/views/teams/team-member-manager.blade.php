                        ...
                        <div class="col-lg-12">
                            ...
                            <div class="relative z-0 mt-1 rounded-lg cursor-pointer">
                                @foreach ($this->roles as $index => $role)
                                    <button type="button" class="relative px-4 py-3 btn-sm btn {{ $index > 0 ? '' : '' }} {{ ! $loop->last ? 'rounded-b-none' : '' }}"
                                                    wire:click="$set('addTeamMemberForm.role', '{{ $role->key }}')">
                                        <div class="{{ isset($addTeamMemberForm['role']) && $addTeamMemberForm['role'] !== $role->key ? 'opacity-50' : '' }}">
                                            <!-- Role Name -->
                                            <div class="row items-center text-sm">
                                                <span class="text-sm text-gray-600 {{ $addTeamMemberForm['role'] == $role->key ? 'font-semibold' : '' }}">
                                                    {{ $role->name }}
                                                </span>

                                                @if ($addTeamMemberForm['role'] == $role->key)
                                                <span class="span">
                                                    <svg style="width: 50px; height:50px" class="text-primary text-sm" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <x-slot name="content">
                    <div class="row">
                        @foreach ($team->teamInvitations as $invitation)
                            <div class="flex col-xl-4 col-lg-4 items-center justify-between">
